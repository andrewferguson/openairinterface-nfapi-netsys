...
int pnf_config_resp_cb(nfapi_vnf_config_t *config, int p5_idx, nfapi_pnf_config_response_t *resp) {
  ...
  if(1) {
    ...
    req.header.phy_id = resp->header.phy_id;
    req.header.message_id = NFAPI_PNF_START_REQUEST;
     NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] phy_id : %d, message_id : %d \n", resp->header.phy_id, NFAPI_PNF_START_REQUEST);
...
void configure_nfapi_vnf(char *vnf_addr, int vnf_p5_port, char *pnf_ip_addr, int pnf_p7_port, int vnf_p7_port, int *enb_id) {
  printf("enb_id : %d \n",*enb_id);
  uint16_t id = *enb_id;
  nfapi_setmode(NFAPI_MODE_VNF);
  memset(&vnf, 0, sizeof(vnf));
  ...
  vnf.p7_vnfs[0].local_port = vnf_p7_port;
  vnf.p7_vnfs[0].mac = (mac_t *)malloc(sizeof(mac_t));
  nfapi_vnf_config_t *config = nfapi_vnf_config_enbid_create(id);
  config->malloc = malloc;
  config->free = free;
  config->vnf_p5_port = vnf_p5_port;
  config->vnf_ipv4 = 1;
...
int oai_nfapi_dl_config_req(nfapi_dl_config_request_t *dl_config_req) {
  ...
  // printf("phy id in dl config %d \n",vnf.p7_vnfs[0].config->phy_id );
